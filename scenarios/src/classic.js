...
const resolveSpecialImgUrl = (definition, specialImageIndex) => {
...
const resolveChoiceKindFromTargeting = (definition) => {
    ...
    if (targeting.includes("bunker") || targeting.includes(tClassic("classic.auto.001")))
        return "bunker";
    if (targeting.includes("category") || targeting.includes(tClassic("classic.auto.033")))
        return "category";
...
const resolveTargetScope = (definition) => {
    ...
    if (choiceKind === "category" || choiceKind === "bunker")
        return null;
...
export const scenario = {
    ...
    createSession(ctx) {
        ...
        const emitEvent = (kind, messageOrKey, vars, explicitMessage) => {
        ...
        const setStageText = (messageOrKey, vars, explicitMessage) => {
        ...
        const getVoteReasonKey = (reasonCode) => {
            switch (reasonCode) {
                case "VOTE_BLOCKED":
        ...
        const markVoteWasted = (state, voterId, reason, reasonCode = "VOTE_BLOCKED") => {
        ...
        const markVoteForcedSelf = (state, voterId, reason, reasonCode = "VOTE_FORCED_SELF") => {
        ...
        const buildWorldView = () => ({
            ...
            bunker: world.bunker.map((card, index) => card.isRevealed ? card : maskWorldCard(card, `${DECK_LABELS.bunker} #${index + 1}`)),
            threats: world.threats.map((card, index) => card.isRevealed ? card : maskWorldCard(card, tClassicFmt("classic.auto.120", { v1: index + 1 }))),
            counts: world.counts,
        });
        ...
        const resolveBunkerIndex = (payload, { allowRandom = false } = {}) => {
            ...
            if (rawIndex === undefined || rawIndex === null || rawIndex === "") {
                if (!allowRandom)
                    return scenarioError("classic.auto.064");
                const randomIndex = getRandomRevealedBunkerIndex();
                if (randomIndex === null)
                    return scenarioError("classic.auto.059");
                return { index: randomIndex };
            }
            const index = Number(rawIndex);
            if (!Number.isInteger(index))
                return scenarioError("classic.auto.048");
            if (index < 0 || index >= world.bunker.length)
                return scenarioError("classic.auto.048");
            if (!world.bunker[index]?.isRevealed)
                return scenarioError("classic.auto.034");
            return { index };
        };
        ...
        const devSkipRound = (actorId) => {
            if (actorId !== ctx.hostId)
                return scenarioError("classic.auto.097");
            if (phase === "voting" || phase === "resolution") {
                return scenarioError("classic.auto.055");
            }
            if (phase === "ended")
                return scenarioError("classic.auto.019");
            for (const player of alivePlayers()) {
        ...
        const finalizeVotingResolution = () => {
            if (!votingState)
                return scenarioError("classic.auto.015");
            clearVoteWindowTimer();
            const source = getVoteSource();
            if (!source)
                return scenarioError("classic.auto.056");
        ...
        const markLeftBunker = (targetId) => {
            ...
            if (!target || target.status === "left_bunker")
                return scenarioError("classic.auto.021");
        ...
        const devKickPlayer = (actorId, targetId) => {
            if (actorId !== ctx.hostId)
                return scenarioError("classic.auto.094");
            if (phase === "ended")
                return scenarioError("classic.auto.019");
            const target = players.get(targetId);
            if (!target)
            ...
            if (target.status !== "alive")
                return scenarioError("classic.auto.022");
        ...
        const pickDeckCard = (deckName, replacementMode, replacementCardId) => {
            ...
            if (replacementMode === "specific") {
                ...
                if (!requested)
                    return scenarioError("classic.auto.035");
                const selected = deck.find((card) => card.id === requested);
                if (!selected)
                    return scenarioError("classic.auto.011");
                return { card: selected };
            }
            const selected = deck[Math.floor(rng() * deck.length)] ?? null;
            if (!selected)
                return scenarioError("classic.auto.038");
            return { card: selected };
        };
        const adminReplacePlayerCard = (actorId, payload) => {
            if (actorId !== ctx.hostId)
                return scenarioError("classic.auto.089");
            ...
            if (targetArea === "special") {
                ...
                if (!special)
                    return scenarioError("classic.auto.071");
                ...
                if (!definition)
                    return scenarioError("classic.auto.037");
                if (!definition.implemented)
                    return scenarioError("classic.auto.012");
            ...
            if (!card)
                return scenarioError("classic.auto.030");
        ...
        const adminSetWorldCardReveal = (actorId, payload) => {
            if (actorId !== ctx.hostId)
                return scenarioError("classic.auto.090");
            const index = Number(payload.index);
            if (!Number.isInteger(index) || index < 0)
                return scenarioError("classic.auto.051");
            const list = payload.kind === "bunker" ? world.bunker : world.threats;
            const target = list[index];
            if (!target)
                return scenarioError("classic.auto.031");
        ...
        const adminReplaceWorldCard = (actorId, payload) => {
            ...
            if (!deckName)
                return scenarioError("classic.auto.040");
        ...
        const adminSetWorldCount = (actorId, payload) => {
            if (actorId !== ctx.hostId)
                return scenarioError("classic.auto.091");
            const count = Number(payload.count);
            if (!Number.isInteger(count) || count < 0)
                return scenarioError("classic.auto.050");
        ...
        const adminApplySpecial = (actorId, payload) => {
            if (actorId !== ctx.hostId)
                return scenarioError("classic.auto.092");
            const sourcePlayer = players.get(payload.actorPlayerId);
            if (!sourcePlayer)
                return scenarioError("classic.auto.023");
            ...
            if (specialInstanceId) {
                ...
                if (result.error)
                    return result;
                emitEvent("info", tClassicFmt("classic.auto.136", { v1: sourcePlayer.name, v2: special ? `: ${special.definition.title}` : "" }));
                return result.stateChanged ? { stateChanged: true } : result;
            }
            const specialId = String(payload.specialId ?? "").trim();
            if (!specialId)
                return scenarioError("classic.auto.068");
            const definition = findSpecialDefinitionForAdmin(specialId);
            if (!definition)
                return scenarioError("classic.auto.087");
            if (!definition.implemented)
                return scenarioError("classic.auto.013");
            const tempSpecial = {
            ...
            if (result.error)
                return result;
            emitEvent("info", sourcePlayer.playerId === ctx.hostId
        ...
        const revealCard = (player, cardId) => {
            if (phase === "ended") {
                ...
                if (!cardEnded)
                    return scenarioError("classic.auto.032");
                if (cardEnded.revealed)
                    return scenarioError("classic.auto.111");
                cardEnded.revealed = true;
                return { stateChanged: true };
            }
            if (phase !== "reveal")
                return scenarioError("classic.auto.079");
            ...
            if (player.revealedThisRound)
                return scenarioError("classic.auto.010");
        ...
        const continueRound = (player) => {
            if (phase !== "reveal_discussion")
                return scenarioError("classic.auto.078");
        ...
        const applySpecial = (player, specialInstanceId, payload) => {
            if (player.status !== "alive")
                return scenarioError("classic.auto.006");
            const special = player.specialConditions.find((item) => item.instanceId === specialInstanceId);
            if (!special)
                return scenarioError("classic.auto.072");
            if (!special.definition.implemented)
                return scenarioError("classic.auto.108");
            if (special.used)
                return scenarioError("classic.auto.110");
            ...
            if (targetScope) {
                if (targetScope === "neighbors") {
                    ...
                    if (!("neighborId" in neighborChoice))
                    ...
                    if (candidates.length === 0)
                        return scenarioError("classic.auto.058");
                    const targetId = String(payload.targetPlayerId ?? "");
                    if (!targetId)
                        return scenarioError("classic.auto.069");
                    if (!candidates.includes(targetId))
                        return scenarioError("classic.auto.041");
                }
            }
            ...
            for (const requirement of requires) {
                ...
                if (requirement === "targetHasBaggage") {
                    ...
                    if (!target || !hasBaggage)
                        return scenarioError("classic.auto.098");
                }
                if (requirement === "targetHasRevealedHealth") {
                    ...
                    if (!target || !hasRevealed)
                        return scenarioError("classic.auto.099");
                }
                if (requirement === "targetHasRevealedProfession") {
                    ...
                    if (!target || !hasRevealed)
                        return scenarioError("classic.auto.100");
                }
                if (requirement === "targetHasRevealedSameCategory") {
                    ...
                    if (!deckName)
                        return scenarioError("classic.auto.045");
                    const neighborChoice = resolveNeighborChoice(player.playerId, payload);
                    if (!("neighborId" in neighborChoice))
                        return neighborChoice;
                ...
                if (requirement === "trackFirstRevealHealth") {
                    if (!firstHealthRevealPlayerId)
                        return scenarioError("classic.auto.017");
                }
            }
            return null;
        };
        ...
        const applySpecialEffect = (player, special, payload) => {
            ...
            switch (effectType) {
                case "banVoteAgainst": {
                    ...
                    const target = players.get(targetId);
                    if (!target || target.status !== "alive")
                        return scenarioError("classic.auto.107");
                ...
                case "forceRevote": {
                    ...
                    if (def.effect.params?.disallowPreviousCandidate) {
                        const disallowByVoter = new Map();
                        for (const [voterId, record] of source.entries()) {
                            if (!record.isValid || !record.targetId)
                                continue;
                            if (!votingState.candidates.has(record.targetId))
                                continue;
                ...
                case "replaceRevealedCard": {
                    ...
                    if (!revealedCard)
                        return scenarioError("classic.auto.103");
                    const newCard = drawCardFromDeck(deckName, deckPools, rng);
                    if (!newCard)
                        return scenarioError("classic.auto.148", { v1: deckName });
                ...
                case "redealAllRevealed": {
                    ...
                    if (revealedSlots.length === 0)
                        return scenarioError("classic.auto.060");
                ...
                case "replaceBunkerCard": {
                    ...
                    if (!replacement)
                        return scenarioError("classic.auto.057");
                ...
                case "forceRevealCategoryForAll": {
                    ...
                    if (!deckName)
                        return scenarioError("classic.auto.065");
                ...
                case "stealBaggage_and_giveSpecial": {
                    ...
                    if (targetBaggage.length === 0)
                        return scenarioError("classic.auto.102");
                    ...
                    if (!stolenCard)
                        return scenarioError("classic.auto.066");
                    ...
                    const specialAssetId = special.imgUrl && special.imgUrl.startsWith("/assets/")
        ...
        const finalizeVotingWindow = () => {
            ...
            if (votePhase !== "voteSpecialWindow")
                return scenarioError("classic.auto.070");
            clearVoteWindowTimer();
        ...
        const buildPublicCategories = (player) => {
            return CATEGORY_ORDER.map((category) => {
                if (category === SPECIAL_CATEGORY) {
                    const revealedSpecialCards = player.specialConditions
                    ...
                    const proxySpecialCards = player.specialCategoryProxyCards.map((card, index) => ({
                        labelShort: card.labelShort,
                        imgUrl: card.imgUrl,
                    ...
                    const cards = revealedSpecialCards.concat(proxySpecialCards);
                    return {
                        category,
                ...
                const allCards = player.hand.filter((card) => cardMatchesDeck(card, deckInfo.deck) &&
