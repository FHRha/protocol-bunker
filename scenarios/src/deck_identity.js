...
export const DECK_ID_TO_DEFAULT_LABEL = {
...
const normalizeKey = (value) => String(value ?? "")
    ...
    .replace(/[\-_]+/g, " ")
    .replace(/\s+/g, " ");
const inferDeckIdFromAssetPath = (assetId) => {
...
export const resolveDeckIdByLabel = (value) => {
    const normalized = normalizeKey(String(value ?? ""));
    if (!normalized)
        return undefined;
    return DECK_LABEL_TO_ID[normalized];
};
export const resolveAssetDeckId = (card) => {
    if (!card)
        return undefined;
...
export const buildDeckAccess = (assets) => {
    ...
    const getDeckCards = (deckId, fallbackDeckName) => {
        ...
        if (fallbackKey) {
            ...
            if (byFallback)
                return byFallback.cards;
        }
        return [];
    };
    const getDeckName = (deckId, fallbackDeckName) => {
        const normalizedDeckId = resolveDeckIdByLabel(deckId) ?? deckId;
        const byDeckId = byId.get(normalizedDeckId);
        if (byDeckId)
            return byDeckId.deckName;
