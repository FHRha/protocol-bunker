...
const CORE_DECKS = [tDev("deck.profession"), tDev("deck.health"), tDev("deck.hobby"), tDev("deck.baggage"), tDev("deck.biology")];
...
const normalizeSpecialLookup = (value) => String(value ?? "")
    .trim()
...
const resolveChoiceKindFromTargeting = (definition) => {
    ...
    if (definition.effect.type === "replaceBunkerCard" ||
    ...
    if (targeting.includes("bunker") || targeting.includes(tDev("parse.target.bunker")))
        return "bunker";
    ...
    if (targeting.includes("category") || targeting.includes(tDev("parse.target.category")))
        return "category";
...
const resolveTargetScope = (definition) => {
    ...
    if (choiceKind === "category" || choiceKind === "special" || choiceKind === "bunker")
        return null;
...
const DEV_SPECIAL_OPTIONS = (() => {
    ...
    for (const item of IMPLEMENTED_SPECIALS) {
        if (!item.id || item.effect.type === DEV_CHOICE_EFFECT_TYPE)
            continue;
...
const buildDevChoiceDefinition = (playerId) => {
    if (DEV_SPECIAL_OPTIONS.length === 0)
        return null;
    return {
...
function cardMatchesDeck(card, deckNameOrId) {
...
export const scenario = {
    ...
    createSession(ctx) {
        ...
        const specialImageIndex = new Map();
        ...
        let lastEliminated;
        let resolutionNote;
        let resolutionNoteKey;
        let resolutionNoteVars;
        let winners;
        let resolutionTimer;
        let voteWindowTimer;
        let discussionTimer;
        ...
        let lastStageText;
        let lastStageTextKey;
        let lastStageTextVars;
        let roundRules = {};
        let firstHealthRevealPlayerId;
        let currentTurnPlayerId = null;
        let lastRevealerId = null;
        const finalThreats = [];
        const world = rollWorldFromAssets(ctx.assets, rng, ctx.players.length, ctx.settings.forcedDisasterId);
        const bunkerDeckAssets = deckAccess.getDeckCards("bunker", tDev("deck.bunker"));
        const initialBunkerIds = new Set(world.bunker.map((card) => card.id).filter((id) => Boolean(id)));
        const bunkerReplacementPool = bunkerDeckAssets.length > 0
            ? bunkerDeckAssets
        ...
        let worldEvent = null;
        const resolveSpecialImgUrl = (definition) => {
        ...
        const resolveSpecialAssetId = (definition) => {
        ...
        const scenarioError = (key, vars) => ({
            error: vars ? tDevFmt(key, vars) : tDev(key),
            errorKey: key,
            errorVars: vars,
        });
        const emitEvent = (kind, messageOrKey, vars, options) => {
        ...
        const setStageText = (messageOrKey, vars, options) => {
            const isKey = options?.isKey ?? true;
        ...
        const removeDevPlayer = (targetId) => {
            ...
            if (!idToRemove)
                return scenarioError("error.removePlayer.none");
            const player = players.get(idToRemove);
            if (!player)
                return scenarioError("error.player.notFound");
        ...
        const autoRevealBots = () => {
            ...
            let candidate = forcedCategory
                ? player.hand.find((card) => cardMatchesDeck(card, deckInfo?.deck ?? forcedCategory) &&
        ...
        const getCardsByCategoryKey = (player, categoryKey, onlyRevealed = false) => {
            ...
            return player.hand.filter((card) => {
                if (!cardMatchesDeck(card, deckName))
                    return false;
                if (slotKey && card.slotKey !== slotKey)
                    return false;
                if (onlyRevealed && !(DEV_SHOW_ALL_PUBLIC || card.revealed))
                    return false;
                return true;
            });
        };
        ...
        const getSelectedRevealedCard = (player, categoryKey, requestedInstanceId) => {
        ...
        const markVoteWasted = (state, voterId, reason, reasonCode = "VOTE_BLOCKED", reasonKey, reasonVars) => {
        ...
        const markVoteForcedSelf = (state, voterId, reason, reasonCode = "VOTE_FORCED_SELF", reasonKey, reasonVars) => {
        ...
        const isBunkerCardPubliclyRevealed = (card) => Boolean(card && (card.isRevealed || DEV_SHOW_ALL_PUBLIC));
        const getRevealedBunkerIndices = () => world.bunker
            .map((card, index) => (isBunkerCardPubliclyRevealed(card) ? index : -1))
            .filter((index) => index >= 0);
        const getRandomRevealedBunkerIndex = () => {
        ...
        const resolveBunkerIndex = (payload, { allowRandom = false } = {}) => {
            const rawIndex = payload.bunkerIndex;
            if (rawIndex === undefined || rawIndex === null || rawIndex === "") {
                if (!allowRandom)
                    return scenarioError("error.bunker.pickRequired");
                const randomIndex = getRandomRevealedBunkerIndex();
                if (randomIndex === null)
                    return scenarioError("error.bunker.noRevealed");
                return { index: randomIndex };
            }
            const index = Number(rawIndex);
            if (!Number.isInteger(index))
                return scenarioError("error.bunker.invalid");
            if (index < 0 || index >= world.bunker.length)
                return scenarioError("error.bunker.invalid");
        ...
        const pickBunkerReplacementCard = (excludeIds) => {
            if (bunkerReplacementPool.length === 0)
                return null;
            const candidateIndices = bunkerReplacementPool
                .map((card, index) => (!excludeIds.has(card.id) ? index : -1))
                .filter((index) => index >= 0);
            const sourceIndices = candidateIndices.length > 0 ? candidateIndices : bunkerReplacementPool.map((_, index) => index);
        ...
        const devSkipRound = (actorId) => {
            if (actorId !== ctx.hostId)
                return scenarioError("error.host.onlySkipRound");
            if (phase === "voting" || phase === "resolution") {
                return scenarioError("error.skipRound.voting");
            }
            if (phase === "ended")
                return scenarioError("error.game.alreadyEnded");
            for (const player of alivePlayers()) {
                player.revealedThisRound = true;
            }
            advanceAfterDiscussion();
        ...
        const pickDeckCard = (deckName, replacementMode, replacementCardId) => {
            const deckId = resolveDeckIdByLabel(deckName) ?? deckName;
            const deck = deckAccess.getDeckCards(deckId, deckName);
            if (deck.length === 0)
                return { card: undefined, ...scenarioError("error.deck.unavailable", { deckName }) };
            if (replacementMode === "specific") {
                const requested = String(replacementCardId ?? "").trim();
                if (!requested)
                    return { card: undefined, ...scenarioError("error.replace.pickSpecificCard") };
                const selected = deck.find((card) => card.id === requested);
                if (!selected)
                    return { card: undefined, ...scenarioError("error.replace.selectedCardMissing") };
                return { card: selected };
            }
            const selected = deck[Math.floor(rng() * deck.length)] ?? null;
            if (!selected)
                return { card: undefined, ...scenarioError("error.replace.randomFailed") };
            return { card: selected };
        };
        const adminReplacePlayerCard = (actorId, payload) => {
            if (actorId !== ctx.hostId)
                return scenarioError("error.host.onlyPlayerCards");
            ...
            if (targetArea === "special") {
                const special = target.specialConditions.find((entry) => entry.instanceId === payload.cardInstanceId);
                if (!special)
                    return scenarioError("error.special.playerMissing");
                const definition = payload.replacementMode === "specific"
                ...
                if (!definition)
                    return scenarioError("error.special.replaceFailed");
                if (!definition.implemented)
                    return scenarioError("error.special.notImplemented");
            ...
            if (!card)
                return scenarioError("error.playerCard.notFound");
            const picked = pickDeckCard(card.deck, payload.replacementMode, payload.replacementCardId);
            if (!picked.card)
                return picked.error ? { error: picked.error, errorKey: picked.errorKey, errorVars: picked.errorVars } : scenarioError("error.playerCard.replaceFailed");
        ...
        const detectWorldDeckName = (kind) => {
            const entries = Object.entries(ctx.assets.decks ?? {});
            if (entries.length === 0)
                return null;
            const worldIds = kind === "bunker"
        ...
        const adminSetWorldCardReveal = (actorId, payload) => {
            if (actorId !== ctx.hostId)
                return scenarioError("error.host.onlyWorldCards");
            const index = Number(payload.index);
            if (!Number.isInteger(index) || index < 0)
                return scenarioError("error.world.invalidIndex");
            const list = payload.kind === "bunker" ? world.bunker : world.threats;
            const target = list[index];
            if (!target)
                return scenarioError("error.world.cardNotFound");
        ...
        const adminReplaceWorldCard = (actorId, payload) => {
            if (actorId !== ctx.hostId)
                return scenarioError("error.host.onlyWorldCards");
            const deckName = detectWorldDeckName(payload.kind);
            if (!deckName)
                return scenarioError("error.world.deckUnknown");
            const picked = pickDeckCard(deckName, payload.replacementMode, payload.replacementCardId);
            if (!picked.card)
                return picked.error ? { error: picked.error, errorKey: picked.errorKey, errorVars: picked.errorVars } : scenarioError("error.world.replaceFailed");
        ...
        const adminSetWorldCount = (actorId, payload) => {
            if (actorId !== ctx.hostId)
                return scenarioError("error.host.onlyWorldCards");
            const count = Number(payload.count);
            if (!Number.isInteger(count) || count < 0)
                return scenarioError("error.world.invalidCount");
        ...
        const findSpecialDefinitionForAdmin = (specialIdRaw) => {
            const lookup = String(specialIdRaw ?? "").trim();
            if (!lookup)
                return null;
            const normalized = normalizeSpecialLookup(lookup);
            const byId = SPECIAL_DEFINITIONS.find((definition) => normalizeSpecialLookup(definition.id) === normalized ||
                normalizeSpecialLookup(toSpecialFileName(definition.id)) === normalized);
            if (byId)
                return byId;
            const byFile = SPECIAL_DEFINITIONS.find((definition) => normalizeSpecialLookup(definition.file) === normalized ||
                normalizeSpecialLookup(toSpecialFileName(definition.file)) === normalized);
            if (byFile)
                return byFile;
            const byTitle = SPECIAL_DEFINITIONS.find((definition) => normalizeSpecialLookup(definition.title) === normalized);
            return byTitle ?? null;
        };
        const adminApplySpecial = (actorId, payload) => {
            if (actorId !== ctx.hostId)
                return scenarioError("error.host.onlyManualSpecial");
            const sourcePlayer = players.get(payload.actorPlayerId);
            if (!sourcePlayer)
                return scenarioError("error.sourcePlayer.notFound");
            const effectivePayload = (payload.payload ?? {});
            ...
            if (specialInstanceId) {
                ...
                if (result.error)
                    return result;
                emitEvent("info", "event.host.appliedPlayerSpecial", { player: sourcePlayer.name, suffix: special ? tDevFmt("event.host.appliedPlayerSpecial.suffix", { title: special.definition.title }) : "" });
                return result.stateChanged ? { stateChanged: true } : result;
            }
            const specialId = String(payload.specialId ?? "").trim();
            if (!specialId)
                return scenarioError("error.special.pickRequired");
            const definition = findSpecialDefinitionForAdmin(specialId);
            if (!definition)
                return scenarioError("error.special.catalogMissing");
            if (!definition.implemented)
                return scenarioError("error.special.notImplemented");
            const tempSpecial = {
        ...
        const finalizeVotingResolution = () => {
            if (!votingState)
                return scenarioError("error.voting.notStarted");
            clearVoteWindowTimer();
            const source = getVoteSource();
            if (!source)
                return scenarioError("error.voting.noSource");
        ...
        const revealCard = (player, cardId) => {
            if (phase !== "reveal")
                return scenarioError("error.reveal.notNow");
            ...
            if (player.revealedThisRound)
                return scenarioError("error.reveal.alreadyThisRound");
            const card = getCardByInstanceId(player, cardId);
            if (!card)
                return scenarioError("error.card.notFound");
            if (card.revealed)
                return scenarioError("error.card.alreadyRevealed");
            if (roundRules.forcedRevealCategory) {
                const forcedCategory = roundRules.forcedRevealCategory;
                const deckInfo = CATEGORY_LABEL_TO_DECK[forcedCategory];
                const hasForcedHidden = deckInfo
                    ? player.hand.some((entry) => cardMatchesDeck(entry, deckInfo.deck) &&
        ...
        const continueRound = (player) => {
            if (phase !== "reveal_discussion")
                return scenarioError("error.continue.notNow");
        ...
        const isDevChoiceSpecial = (special) => special.definition.id.startsWith("dev-choice-");
        const buildDevSpecialFromTemplate = (currentSpecial, template) => ({
            ...template,
        ...
        const applySpecial = (player, specialInstanceId, payload) => {
            if (player.status !== "alive")
                return scenarioError("error.player.excluded");
            const special = player.specialConditions.find((item) => item.instanceId === specialInstanceId);
            if (!special)
                return scenarioError("error.special.notFound");
            if (!special.definition.implemented)
                return scenarioError("error.special.unimplemented");
            if (special.used)
                return scenarioError("error.special.alreadyUsed");
            ...
            if (targetScope) {
                if (targetScope === "neighbors") {
                    ...
                    if (candidates.length === 0)
                        return scenarioError("error.target.none");
                    const targetId = String(payload.targetPlayerId ?? "");
                    if (!targetId)
                        return scenarioError("error.target.required");
                    if (!candidates.includes(targetId))
                        return scenarioError("error.target.invalid");
                }
            }
            ...
            for (const requirement of requires) {
                ...
                if (requirement === "targetHasBaggage") {
                    ...
                    if (!target || !hasBaggage)
                        return scenarioError("validate.target.noBaggage");
                }
                if (requirement === "targetHasRevealedHealth") {
                    ...
                    if (!target || !hasRevealed)
                        return scenarioError("validate.target.noRevealedHealth");
                }
                if (requirement === "targetHasRevealedProfession") {
                    ...
                    const hasRevealed = target
                        ...
                        : false;
                    if (!target || !hasRevealed)
                        return scenarioError("validate.target.noRevealedProfession");
                }
                if (requirement === "targetHasRevealedSameCategory") {
                    ...
                    if (!deckName)
                        return scenarioError("validate.category.unknown");
                    const neighborChoice = resolveNeighborChoice(player.playerId, payload);
                    if ("error" in neighborChoice)
                        return neighborChoice;
                    ...
                    const hasRevealed = (DEV_SHOW_ALL_PUBLIC ? getAnyCardsByCategory(neighbor, deckName) : getRevealedCardsByCategory(neighbor, deckName))
                        .length > 0;
                    if (!hasRevealed)
                        return scenarioError("validate.neighbor.noRevealedCard");
                }
                if (requirement === "needsNeighborIndexing") {
                    if (playerOrder.length <= 1)
                        return scenarioError("validate.neighbor.notEnoughPlayers");
                }
                if (requirement === "ageFieldAvailable") {
                    const ages = alivePlayers().map((p) => getRevealedAge(p)).filter((age) => age !== null);
                    if (ages.length === 0)
                        return scenarioError("validate.age.noneRevealed");
                }
                ...
                if (requirement === "trackFirstRevealHealth") {
                    if (!firstHealthRevealPlayerId)
                        return scenarioError("validate.health.firstRevealMissing");
                }
            }
            return null;
        };
        ...
        const applySpecialEffect = (player, special, payload) => {
            ...
            switch (effectType) {
                case DEV_CHOICE_EFFECT_TYPE: {
                    ...
                    if (!selectedId)
                        return scenarioError("error.special.choiceRequired");
                    const selectedDefinition = IMPLEMENTED_SPECIALS.find((item) => item.id === selectedId);
                    if (!selectedDefinition)
                        return scenarioError("error.special.choiceNotFound");
                ...
                case "banVoteAgainst": {
                    ...
                    const target = players.get(targetId);
                    if (!target || target.status !== "alive")
                        return scenarioError("error.target.notAlive");
                ...
                case "replaceRevealedCard": {
                    ...
                    if (!revealedCard)
                        return scenarioError("error.target.noRevealedCategory");
                    const newCard = drawCardFromDeck(deckName, deckPools, rng);
                    if (!newCard)
                        return scenarioError("error.deck.emptyCategory", { deckName });
                ...
                case "redealAllRevealed": {
                    ...
                    if (revealedSlots.length === 0)
                        return scenarioError("error.redeal.none");
                ...
                case "replaceBunkerCard": {
                    ...
                    const occupiedIds = new Set(world.bunker.map((card) => card.id).filter((id) => Boolean(id)));
                    const replacement = pickBunkerReplacementCard(occupiedIds);
                    if (!replacement)
                        return scenarioError("error.bunker.noReplacement");
                ...
                case "forceRevealCategoryForAll": {
                    ...
                    if (!deckName)
                        return scenarioError("error.category.required");
                ...
                case "stealBaggage_and_giveSpecial": {
                    ...
                    const stolenCard = (requestedBaggageCardId
                        ? targetBaggage.find((card) => card.instanceId === requestedBaggageCardId)
                        : targetBaggage[0]) ?? null;
                    if (!stolenCard)
                        return scenarioError("error.baggage.pickSpecific");
        ...
        const vote = (player, targetId) => {
            if (phase !== "voting" || !votingState)
                return scenarioError("error.voting.notNow");
            if (votePhase !== "voting")
                return scenarioError("error.vote.collectionClosed");
            if (targetId === player.playerId)
                return scenarioError("error.vote.self");
            if (!votingState.candidates.has(targetId))
                return scenarioError("error.vote.invalidCandidate");
            if (votingState.votes.has(player.playerId))
                return scenarioError("error.vote.alreadySubmitted");
            if (votingState.revoteDisallowTargets.has(targetId))
                return scenarioError("error.vote.disallowedCandidate");
            const target = players.get(targetId);
            if (!target || target.status !== "alive")
                return scenarioError("error.vote.candidateNotAlive");
            if (target.bannedAgainst.has(player.playerId)) {
        ...
        const finalizeVotingWindow = () => {
            ...
            if (votePhase !== "voteSpecialWindow")
                return scenarioError("error.vote.specialWindowNotOpen");
            clearVoteWindowTimer();
        ...
        const isPubliclyVisibleCard = (card) => DEV_SHOW_ALL_PUBLIC || card.revealed;
            ...
            return CATEGORY_ORDER.map((category) => {
                if (category === SPECIAL_CATEGORY) {
                    const cards = player.specialConditions
                        ...
                        .concat(player.specialCategoryProxyCards.map((card, index) => ({
                        labelShort: card.labelShort,
                        imgUrl: card.imgUrl,
                ...
                const visibleCards = player.hand.filter((card) => {
                    const matchDeck = cardMatchesDeck(card, deckInfo.deck) && (!deckInfo.slotKey || card.slotKey === deckInfo.slotKey);
                    if (!matchDeck)
                        return false;
        ...
        const buildYouCategories = (player) => {
            return CATEGORY_ORDER.filter((category) => category !== SPECIAL_CATEGORY).map((category) => {
                ...
                const cards = deckInfo
                    ? player.hand
                        .filter((card) => cardMatchesDeck(card, deckInfo.deck) && (!deckInfo.slotKey || card.slotKey === deckInfo.slotKey))
                        .map((card) => ({
                        instanceId: card.instanceId,
                        labelShort: card.labelShort,
                        revealed: card.revealed,
        ...
        return {
            getGameView(playerId) {
                ...
                const disallowedVoteTargetIdsForYou = (() => {
                    if (!votingState || votePhase !== "voting")
                        return undefined;
